...
To fetch the skeleton files for this lab \verb|cd| to the git repository (\verb|fpga_labs_sp19|) that you had cloned in the first lab and execute the command \verb|git pull|.

You can find the documents/datasheets useful for this lab in the \verb|fpga_labs_sp19/resources| folder. Go through the Verilog Primer Slides in \verb|resources/Verilog|; you should feel somewhat comfortable with the basics of Verilog to complete this lab.
...
The best reference for this board is provided by Digilent: \url{https://reference.digilentinc.com/reference/programmable-logic/pynq-z1/reference-manual} (a PDF version of this manual is available in the \verb|fpga_labs_sp19/resources| folder). Browse the documentation there to get a feel for both what features the board has and, more importantly, what information the documentation has, should you need it later.
...
\subsection{Checkoff Questions}\label{sec:checkoffQuestions}
\begin{enumerate}
...
Now that you have cloned the \verb|fpga_labs_sp19| repository, you can \verb|cd| to the 
\verb|fpga_labs_sp19/lab1| directory to see this lab's skeleton files. You will note that there is a \verb|lab1.srcs| directory and a \verb|lab1.xpr| file.
...
Please take a look at \verb|pynq-rm.pdf| in the \verb|fpga_labs_sp19/resources| folder. Read about the clock sources available on the board on page 14. Clock signals are generated outside the FPGA by a crystal oscillator or a programmable clock generator IC. These clock signals are then connected to pin(s) on the FPGA so that they can be used in your Verilog design.
...
You are done with this lab. In the next lab, we will simulate our digital designs in software, and extend our \verb|tone_generator| to read a song from a ROM and play it through our Audio Out.
